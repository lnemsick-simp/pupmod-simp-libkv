  ...
  def atomic_delete(params)
    ...
    unless catalog.respond_to?(:libkv)
      ...
      if File.exists?(filename)
        ...
        raise("Internal error: libkv::atomic_delete unable to load #{filename}: File not found")
      end
    end

    # use libkv for atomic_delete operation
    retval = nil
    begin
      retval = catalog.libkv.atomic_delete(nparams['url'], nparams['auth'], nparams);
    rescue Exception => e
      if nparams['softfail']
        retval = []
      else
